...
| `REAL_HPC_INTEGRATION` | EXTERNAL_VALIDATION_PENDING | synthetic multi-cluster `site.yaml`、fake template library/backend 已覆盖 resolution、attempt path、stage、scheduler reconciliation、fetch 与 scientific check/collect；本次重构没有访问真实站点 | 未验证真实 site config、SSH、template bootstrap/权限、`sbatch`、monitor、fetch、module/launcher/executable 或生产科学程序 |
