...
| `dft-labeling` | 两个独立操作：bundled pymatgen `vasp-prepare` 生成 static/relax/AIMD 输入；`label` 支持 local user wrapper，另有单结构 static `ssh-slurm` 窄合同 | `dft-input-manifest.json`、可收集的 POSCAR/INCAR/KPOINTS；标准标签与 raw-output hashes；远端 basename staging/fetch allowlist | SI/历史单点模板参数只读 source audit；真实 pymatgen 生成 smoke；SSH-SLURM fresh stage/submit/reconcile/bounded fetch/pinned check fake E2E | POTCAR 只从 `PMG_VASP_PSP_DIR` 组装；远端可上传但永不 fetch/collect/入库；真实 VASP/HPC 尚未运行、无数值 parity；relax/AIMD scheduler、array/continuation pending |
