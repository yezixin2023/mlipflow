...
| `dft-labeling` | 两个独立操作：bundled pymatgen `vasp-prepare` 生成 static/relax/AIMD 输入；`label` 支持 local user wrapper，static/relax/AIMD VASP 均可向通用 `ssh-slurm` backend 提供 scientific stage/fetch/check/collect 合同 | `dft-input-manifest.json`、可收集的 POSCAR/INCAR/KPOINTS；逐帧标准标签、AIMD `vasprun.xml` 轨迹角色与 raw-output 路径；远端 input/output/logs/completion allowlist | SI/历史单点参数只读 source audit；真实 pymatgen 生成 smoke；fake backend 覆盖完整 AIMD scheduler lifecycle 与逐帧收敛/计数；真实 CPU VASP bounded round 中两个 static label checker/collect=`OK`，另一个因电子未收敛正确为 `FAIL` | POTCAR 只从站点授权环境组装且永不 fetch/collect/入库；当前真实验证仅覆盖 large-cell static、单节点 128 MPI ranks、`NCORE=4`，无 VASP 数值 parity；真实 relax/AIMD、array/continuation pending |
...
| `mlip-benchmark` | `evaluate-fresh` 通过共享 model runtime 加载六个 exact family，生成 canonical prediction evidence，再复用唯一 MAE/RMSE/Pearson/ranking；`normalize-execute` 重算 supplied pairs，并按显式 task/metric 分开排序任意明确模型集合 | 历史 DeepMD/CHGNet workbook、prepared JSON/CSV/XLSX、论文表；AIMD/MLIP RDF 与输运 comparison records；四份真实 CHGNet immutable-audit prediction evidence | fresh contract/injected-predictor 全链测试完成；2/3/6 模型静态、结构动力学和输运分任务 fixture 完成；三份 DeepMD 与一份 CHGNet 历史 source/output 为 `REPLAY_VERIFIED`；非 DFT training cluster 上四个 fine-tuned CHGNet member 分别对同一六结构 audit 做 fresh inference 并 checker/collect=`OK` | audit 数值未通过当前 policy，且不代表六模型 production parity；M3GNet benchmark output、DPA-2 raw pairs 和论文 RDF 原始曲线为 `MISSING_SOURCE`；历史未声明单位不猜测 |
...
| `ionic-transport` | local-only formal runner：ASE/LAMMPS/VASP trajectory → pymatgen Structure/`DiffusionAnalyzer`；MSD → `get_diffusivity_from_msd`；Structure 可用时 `get_conversion_factor`；Arrhenius → `fit_arrhenius(linear)`；可选同温度 AIMD/MLIP partial RDF 与分任务 comparison records | analysis manifest、runtime/source/result 路径、pymatgen API checker rerun；RDF curve/comparison JSON；历史 stdout/integration manifest | trajectory D/σ/time、MSD-only D/σ、Arrhenius 均与 pymatgen public API parity；合成 VASP AIMD 与 LAMMPS 轨迹共享输运/RDF/checker/benchmark 路径；保留 MACE/DeepMD 历史 parity | formal 需要 `[transport]` 且无 NumPy fallback；RDF 要求显式物种对、bin/window 且上限不超过最小周期胞高的一半；极短轨迹不提供收敛输运数值；论文 RDF 与真实 AIMD/MLIP 对照仍无 source fixture；历史 N=7 仅限隔离 parity convention |
