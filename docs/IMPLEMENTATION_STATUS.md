...
| `REAL_HPC_INTEGRATION` | SCHEDULER_CONTRACT_VERIFIED_ON_ONE_SITE | synthetic multi-cluster `site.yaml`、fake template library/backend 覆盖 resolution、attempt path、stage、scheduler reconciliation、fetch 与 scientific check/collect；**另有一个真实 SSH-SLURM 站点完成 CPU tiny smoke**：真实 site config、SSH、template bootstrap、fresh attempt workspace、逐文件 SHA-256 stage、`sbatch`、`squeue`/`sacct` terminal state、bounded fetch、completion 身份校验、pinned plugin check/collect 全链路到 `OK` | 该 smoke 不加载 module、不调用任何科学程序：VASP/LAMMPS/DeePMD 等**均未在调度器上验证**；GPU、多节点、cancellation、排队/容错、生产科学任务与该站点之外的集群同样未验证 |
