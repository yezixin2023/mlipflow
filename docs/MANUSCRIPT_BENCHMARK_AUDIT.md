...
`EXECUTABLE` 仅表示：给定已存在的参考值/预测值或轨迹后，MLIPFlow
可以在本地重新计算所列数值；它不表示 MLIP 已被加载并产生新预测。
...
|---|---|---|---|---|---|---|---|
| `deepmd-se_e2_a` | test energy RMSE / Pearson r；force RMSE / Pearson r | 历史 DeepMD metrics XLSX；测试集 13,024 个结构、2,673,726 个力分量 | `plugins/mlip-benchmark/benchmark_wrapper.py` 的 DeepMD workbook parser | replay | XLSX、精确模型名、任务/场景、energy=`eV/atom`、force=`eV/angstrom` | REPLAYABLE | 若要称为新评测，仍需提供参考/预测对或可运行模型；当前重放值分别为 0.007190632639714127、0.998383122897441、0.1766770059878328、0.9564072093736768 |
| `deepmd-se_e2_r` | test energy RMSE / Pearson r；force RMSE / Pearson r | 历史 DeepMD metrics XLSX；同一测试划分 | 同上 | replay | 同上 | REPLAYABLE | 当前重放值分别为 0.01303367441338188、0.9946815122841708、0.2756778043936335、0.8901533484123095；新评测仍需模型或成对预测 |
| `deepmd-se_atten_v2` | test energy RMSE / Pearson r；force RMSE / Pearson r | 历史 DeepMD metrics XLSX；同一测试划分 | 同上 | replay | 同上 | REPLAYABLE | 当前重放值分别为 0.00790596295510387、0.9980456192199905、0.1856936082906573、0.9517273261398933；新评测仍需模型或成对预测 |
| `deepmd-dpa2` | test energy RMSE；force RMSE | SI 中表题为 Table 3 的汇总表 | prepared JSON/CSV replay + 通用排序器 | replay | 紧凑转录、来源文档 SHA-256、单位和样本语义 | REPLAYABLE | 可重建论文舍入值 6.63 meV/atom、151 meV/Å；没有可分享的原始预测对，不能重算相关系数或误差分布 |
| `m3gnet` | test energy RMSE；force RMSE | SI Table 3；另有历史评测脚本，但未取得对应真实输出 workbook | prepared evidence replay | replay | 紧凑转录、来源文档 SHA-256、单位 | REPLAYABLE | 可重放论文值 15.84 meV/atom、212 meV/Å；仍缺真实输出 workbook/预测对来验证脚本级数值 |
| `chgnet` | test energy/force/stress MAE、RMSE、Pearson r | 历史 `efs_metrics_by_split.xlsx` | CHGNet workbook parser | replay | XLSX、任务/场景；energy 单位显式为 `eV/atom` | REPLAYABLE | energy RMSE=0.0080861044340676 eV/atom；force RMSE=0.1889819035175336、stress RMSE=12.46909130171059。源表未可靠声明 force/stress 单位，跨模型物理比较前必须补单位来源，不能猜测 |
...
| `deepmd-se_atten_v2` | 247 个 Li10 大超胞候选的 300 K conductivity ranking / top-k | 历史候选清单与 conductivity 结果；候选 ID 的元素顺序与 metric ID 顺序不同 | `normalize_legacy.py` + `screen.py` + adapter 独立复核 | replay / deterministic rank | 候选与结果的紧凑派生表、显式 `S/m`、排序方向、缺失策略 | REPLAYABLE | top-1 为 `Mn6_Fe3_Ni8_Cu4_Zn7`，18.420176685825222 S/m；候选生成和生产输运推理不在此重放中 |
