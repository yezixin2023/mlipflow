...
| `pes-sampling` — DIRECT | MAML DIRECT `direct.py` | 当前无可分享的历史输入/选择 manifest 对 | 被选结构 ID/索引、cluster assignment | 无可比较结果 | bundled runner/adapter 完成 MAML 调用、argv、fresh-output 和 manifest/check/collect 软件合同 | 未定义 | NOT_TESTABLE_WITH_AVAILABLE_DATA | bundled MAML 路径没有 seed 控制；没有历史同输入选择结果可建立 numerical parity |
...
| `dft-labeling` — VASP prepare | SI Fig. S3 + 历史单点 INCAR/KPOINTS | SI/论文与只读远端模板的 SHA-256 固定参数审计；dev/test interpreter 中真实可 import 的 pymatgen（版本写入 result provenance） | static preset 参数、POSCAR/INCAR/KPOINTS 生成、结构/输入/POTCAR 指纹与 collect 排除策略 | SI：ENCUT=450 eV、EDIFF=5e-6（源标 eV/atom）、IALGO=38；历史模板补充其余 INCAR 与 1×1×1 Monkhorst | `manuscript-static-v1` 分层记录两类来源；fake POTCAR contract 与真实 pymatgen POSCAR/INCAR/KPOINTS 本地 smoke 通过；Adapter check=`OK` | 参数 exact；文件 SHA-256 exact；无 DFT 数值容差 | EXTERNAL_VALIDATION_PENDING | `reports/dft_static_parameter_audit.json` 为 `SOURCE_PARAMETER_AUDIT`；未读取/复制 POTCAR、未运行 VASP，也未验证 scheduler，不建立 energy/force/stress parity；EDIFF 语义差异显式记录 |
| `dft-labeling` — label | 历史 VASP/AIMD 标注脚本 | 当前无小型、已完成、可分享的同输入 DFT fixture | energy/force/stress、收敛、dataset shape | 无可比较结果 | static scientific contract 已接入通用 profile/template/workspace SSH-SLURM lifecycle；synthetic site、fake template/backend 的 stage/fetch/pinned-check E2E 已实现；未运行真实 VASP | 未定义 | NOT_TESTABLE_WITH_AVAILABLE_DATA | 输入生成与 VASP 执行分别审批；scheduler 完成后 fetch/check 再审批；fake XML 只验证软件合同；POTCAR 永不 fetch，真实 OUTCAR/vasprun.xml 尚未形成仓库证据 |
...
| `ionic-transport` — formal pymatgen contract | 合成 MSD、真实 Structure、tiny unwrapped LAMMPS trajectory | local-only；显式 timestep/window/smoothing；3 个温度 | `DiffusionAnalyzer`、`get_diffusivity_from_msd`、`get_conversion_factor`、`fit_arrhenius(linear)`、manifest/checker | 同一 pymatgen public API 直接结果 | runner 保存值与 checker 从原始输入重跑 pymatgen 完全一致；time_ps=`analyzer.dt/1000` | floating-point `isclose` rel=1e-10/abs=1e-14 | PASS | 只验证 pymatgen integration 与 orchestration contract，不证明材料扩散收敛、模型准确性或 Nernst–Einstein 物理适用性 |
...
| `ionic-transport` — historical MD handoff evidence | 历史 `ase_md_only_multi_calc.py` 与当时的 `ionic_conductivity.py` | Li3YCl6；MACE 0.3.15 + 外部 model；ASE 3.28.0、torch 2.10.0、CPU float64、seed=29；400/600/800 K 各 10 steps（1 fs、每 2 步写帧） | historical model load → trajectory → 当时的 NumPy analysis → manifest/check/collect | 既有 smoke rc=0、3 runs/0 failed；没有独立 scientific baseline | 固定历史报告，不冒充当前 formal pymatgen 重跑 | artifact identity exact；未定义科学数值容差 | EXTERNAL_VALIDATION_PENDING | 只证明当时 integration handoff；当前 formal 路径已改为 pymatgen，仍不赋予旧 0.01 ps smoke 收敛或 production 主张 |
| `candidate-ranking` | 历史 Li10 candidate files + conductivity files | 247 个候选的只读结果；元素顺序 `Zn,Fe,Cu,Ni,Mn` 与 metric ID 顺序不同 | candidate identity、247/247 coverage、deterministic top-k | top-1=`Mn6_Fe3_Ni8_Cu4_Zn7`，18.420176685825222 S/m | legacy normalizer + current rank/checker 得到相同 top-1 与稳定排序 | ID exact；metric scalar exact | PASS | 没有重跑 247 个 SQS/MD；这里只比较历史结果规范化与排序。原 evidence 保留 legacy `composition-screening` identity |
| `candidate-ranking` — top validation link | 历史筛选结果与 `6_3_8_4_7` transport artifacts | top-1 composition mapping | top candidate ID 与输运 evidence linkage | 两者指向同一 composition | provenance 可追踪到 top-1 的 transport parity | ID exact + evidence SHA-256 | REPLAY_VERIFIED | 仅连接 MLIP 输运证据；没有 AIMD/DFT 高保真 top-candidate 对照 |
| `candidate-ranking` — high-fidelity validation | 期望的 top candidate AIMD/DFT comparison | 当前无可分享的对应数值表 | ranking agreement / error vs high fidelity | 无 | 无 | 未定义 | EXTERNAL_VALIDATION_PENDING | 这是当前 top-candidate 科学主张的明确缺口 |
