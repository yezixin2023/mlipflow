...
| `ionic-transport` — deterministic contract | 合成线性 MSD、已知 D/T/N/z/V、解析 Arrhenius 序列、tiny unwrapped LAMMPS trajectory | local-only；显式 timestep/fit window；3 个温度 | OLS、`D=slope/(2d)`、Nernst–Einstein、single-line Arrhenius、manifest/checker | 构造真值由解析式定义 | runner 输出经 checker 重新解析 curve 并独立复算一致 | floating-point `isclose` rel=1e-10/abs=1e-14 | PASS | 只验证确定性算法与 orchestration 合同，不证明任何材料的扩散收敛、模型准确性或 Nernst–Einstein 物理适用性 |
