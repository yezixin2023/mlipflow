...
| `dft-labeling` — label | 历史 VASP/AIMD 标注脚本 | 当前无小型、已完成、可分享的同输入 DFT fixture | energy/force/stress、收敛、dataset shape | 无可比较结果 | 严格 result contract、单结构 static SSH-SLURM staging/fetch/pinned-check fake E2E 已实现；未运行真实 VASP | 未定义 | NOT_TESTABLE_WITH_AVAILABLE_DATA | 输入生成与 VASP 执行分别审批；scheduler 完成后 fetch/check 再审批；fake XML 只验证软件合同；POTCAR 永不 fetch，真实 OUTCAR/vasprun.xml 尚未形成仓库证据 |
