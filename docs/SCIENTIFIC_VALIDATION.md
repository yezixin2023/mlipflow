...
| `dft-labeling` — VASP prepare | SI Fig. S3 + 历史单点 INCAR/KPOINTS | SI/论文与只读远端模板的 SHA-256 固定参数审计；真实 pymatgen 2025.10.7 可用环境 | static preset 参数、POSCAR/INCAR/KPOINTS 生成、结构/输入/POTCAR 指纹与 collect 排除策略 | SI：ENCUT=450 eV、EDIFF=5e-6（源标 eV/atom）、IALGO=38；历史模板补充其余 INCAR 与 1×1×1 Monkhorst | `manuscript-static-v1` 分层记录两类来源；fake POTCAR contract 与真实 pymatgen POSCAR/INCAR/KPOINTS 本地 smoke 通过；Adapter check=`OK` | 参数 exact；文件 SHA-256 exact；无 DFT 数值容差 | EXTERNAL_VALIDATION_PENDING | `reports/dft_static_parameter_audit.json` 为 `SOURCE_PARAMETER_AUDIT`；未读取/复制 POTCAR、未运行 VASP、不建立 energy/force/stress parity；EDIFF 语义差异显式记录 |
