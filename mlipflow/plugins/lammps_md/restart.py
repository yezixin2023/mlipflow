...
def _proxy(context: dict[str, Any]) -> dict[str, Any]:
    ...
    copied["parameters"] = {
        key: parameters[key] for key in ("operation", "target") if key in parameters
    }
    return copied
...
def _validate(context: dict[str, Any]) -> list[dict[str, str]]:
    ...
    if unknown:
        diagnostics.append(
            prepare._diagnostic(
                "error",
                "lammps.restart_parameters",
                "unsupported execute parameter(s): " + ", ".join(unknown),
            )
        )
    ...
    if interval is not None and (
        ...
        diagnostics.append(
            prepare._diagnostic(
                "error", "lammps.checkpoint_interval", "checkpoint_interval must be positive"
            )
        )
    ...
    try:
        ...
        diagnostics.append(prepare._diagnostic("error", "lammps.attempt", str(exc)))
    return diagnostics
...
def _runtime_matches(
    ...
    if not execute._plain(runtime.get("lammps_executable")):
...
def _previous_restart(
    ...
    job = prepare._mapping(final_manifest.get("job"))
...
def _plan_execute(context: dict[str, Any]) -> dict[str, Any]:
    ...
    plan = execute.plan(_proxy(context))
    ...
    calculation = prepare._mapping(plan.get("lammps_calculation"))
    ...
    if interval is not None and (
        ...
        return {
            ...
            "diagnostics": [
                prepare._diagnostic(
                    "error",
                    "lammps.checkpoint_interval",
                    "checkpoint_interval cannot exceed approved total steps",
                )
            ],
        }

    scheduled = prepare._mapping(plan.get("scheduled_execution"))
    ...
    if not isinstance(staged, list) or not isinstance(fetch_outputs, list):
        return {
            ...
            "diagnostics": [
                prepare._diagnostic(
                    "error", "lammps.restart_contract", "base scheduled contract is incomplete"
                )
            ],
        }
    ...
    for path, remote_name in (
        ...
        if not execute._ordinary_file(path):
            return {
                "plugin_id": PLUGIN_ID,
                "status": "BLOCKED",
                "executable": False,
                "diagnostics": [
                    prepare._diagnostic(
                        "error", "lammps.restart_helper", f"missing bundled helper: {path.name}"
                    )
                ],
            }
        staged.append(execute._staged(path, remote_name))
    ...
    if interval is not None:
        ...
        salvage = prepare._mapping(plan.get("failure_salvage"))
        names = salvage.get("fetch_remote_names")
        if not isinstance(names, list):
            names = []
        salvage["schema_version"] = 1
        salvage["fetch_remote_names"] = list(
            dict.fromkeys([*names, *CHECKPOINT_FILES, "restart-runtime.json"])
        )
    ...
    if policy == RESTART_AUTO and attempt > 1:
        try:
            ...
            return {
                ...
                "status": "BLOCKED",
                "executable": False,
                "diagnostics": [prepare._diagnostic("error", "lammps.restart_previous", str(exc))],
            }
        staged.append(execute._staged(runtime_path, "restart/restart-runtime.json"))
        candidate_records: dict[str, dict[str, Any]] = {}
        for name, path in sorted(candidates.items()):
            staged.append(execute._staged(path, f"restart/{name}"))
    ...
    if isinstance(paths, dict):
        ...
        for name, record in prepare._mapping(calculation.get("restart_candidates")).items():
            paths[f"restart_{name}"] = record["path"]

    summary = prepare._mapping(plan.get("approval_summary"))
    summary.update(
        {
            "checkpoint_interval": interval,
            "restart_policy": policy,
            ...
            "restart_start_step_resolved_on_compute_node": bool(
                calculation.get("restart_from_attempt")
            ),
...
def _check_restart(context: dict[str, Any]) -> list[dict[str, str]]:
    ...
    try:
        result = execute._read_json(attempt / "lammps-execution-result.json")
    except Exception as exc:
        return [prepare._diagnostic("error", "lammps.restart_result", str(exc))]
    restart = prepare._mapping(result.get("restart"))
    ...
    for key, value in expected.items():
        if restart.get(key) != value:
            diagnostics.append(
                prepare._diagnostic(
                    "error",
                    f"lammps.restart_{key}",
                    f"restart field {key} differs from approved plan",
                )
            )
    ...
    if resumed:
        ...
        if (
            ...
            diagnostics.append(
                prepare._diagnostic(
                    "error",
                    "lammps.restart_start_step",
                    "selected restart timestep is not a valid periodic checkpoint",
                )
            )
        selected = restart.get("selected_checkpoint")
        approved = set(prepare._mapping(calculation.get("restart_candidates")))
        if selected not in approved:
            diagnostics.append(
                prepare._diagnostic(
                    "error",
                    "lammps.restart_checkpoint",
                    "selected restart path was not a staged salvaged candidate",
                )
            )
        if restart.get("lammps_executable") != calculation.get("previous_runtime_executable"):
            diagnostics.append(
                prepare._diagnostic(
                    "error",
                    "lammps.restart_executable",
                    "resume used a different LAMMPS executable path",
                )
            )
        if restart.get("launcher_prefix") != calculation.get("previous_launcher_prefix"):
            diagnostics.append(
                prepare._diagnostic(
                    "error",
                    "lammps.restart_launcher",
                    "resume used different site launcher arguments",
                )
            )
        if restart.get("prepared_launcher") != calculation.get("previous_prepared_launcher"):
            diagnostics.append(
                prepare._diagnostic(
                    "error",
                    "lammps.restart_prepared_launcher",
                    "resume used different prepared launcher arguments",
                )
            )
        if restart.get("platform") != calculation.get("previous_runtime_platform"):
            diagnostics.append(
                prepare._diagnostic(
                    "error",
                    "lammps.restart_platform",
                    "resume platform differs from the salvaged runtime record",
                )
            )
    else:
        if start != 0 or restart.get("selected_checkpoint") is not None:
            diagnostics.append(
                prepare._diagnostic(
                    "error",
                    "lammps.restart_fresh",
                    "fresh execution reported unexpected restart state",
                )
            )
    try:
        report = execute._read_json(attempt / "cluster-run-report.json")
    except Exception as exc:
        diagnostics.append(prepare._diagnostic("error", "lammps.restart_report", str(exc)))
        report = {}
    if (
        ...
        diagnostics.append(
            prepare._diagnostic(
                "error",
                "lammps.restart_report",
                "cluster restart report differs from execution result",
            )
        )
    return diagnostics
