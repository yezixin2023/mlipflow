...
def _validate_generic(context: Any) -> list[dict[str, str]]:
    ...
    if (publish_id is None) != (publish_relative is None):
        ...
        if not _plain(publish_id) or "/" in str(publish_id) or "\\" in str(publish_id):
            diagnostics.append(
...
def _chgnet_completion_diagnostics(
    ...
    predefined_split = (
        ...
        and all(
            isinstance(split.get(key), list) and all(_plain(item) for item in split[key])
...
def _check_generic(
    ...
    if calculation.get("operation") == "finetune":
        ...
        if not isinstance(reported_foundation, Mapping) or not isinstance(
            ...
            or reported_foundation.get("relative_path") != approved_foundation.get("relative_path")
