...
- `plan_digest` 可被绕过、重放到另一计划或审批后被替换；
...
- 批准前检查计划中的 argv、后端、远端目录、资源、输入和摘要。
...
- 内置 Adapter 仅支持 local；scheduled adapter 在固定科学 checker 接入调度回收前由核心拒绝。
- SLURM completion 必须绑定 project/node/run/attempt/plugin/plan digest，并在批准与应用间保持相同文件指纹。
