...
def load_predictor(model: Path, family: str, device: str) -> Predictor:
    """Load one exact family through MLIPFlow's shared scientific runtime."""
...
def _normalizer_module():
    path = Path(__file__).with_name("benchmark_wrapper.py")
    spec = importlib.util.spec_from_file_location("mlip_benchmark_normalizer", path)
    if spec is None or spec.loader is None:
        raise FreshBenchmarkError(f"cannot load metric normalizer: {path}")
