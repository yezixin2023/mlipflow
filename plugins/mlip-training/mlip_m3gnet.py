...
def plan(args, config, data_path):
    if not data_path.is_file():
        raise TrainingError("M3GNet data must be a MatPES JSON/JSONL file")
    section(config, "m3gnet")
    return {
        "framework": "m3gnet",
        "operation": args.operation,
        "python_entrypoint": "matgl.MGLPotentialTrainer",
...
def run(args, config, config_path, data_path):
    ...
    data_cfg = mapping(cfg.get("data"), "m3gnet.data")
    model_cfg = mapping(cfg.get("model"), "m3gnet.model")
    trainer_cfg = mapping(cfg.get("trainer"), "m3gnet.trainer")
    ...
    if args.operation == "finetune":
        ...
        model = model.to(dtype=torch.float64 if args.precision == "float64" else torch.float32)
        elements = tuple(model.element_types)
        mean = getattr(potential, "data_mean", 0.0)
        std = getattr(potential, "data_std", 1.0)
    else:
        elements = tuple(model_cfg.pop("element_types", ())) or None
        model = None
        mean = trainer_cfg.pop("data_mean", 0.0)
        std = trainer_cfg.pop("data_std", 1.0)
    ...
    if model is None:
        elements = elements or tuple(dataset.converter.element_types)
        model_cfg.setdefault("is_intensive", False)
        model = M3GNet(element_types=elements, **model_cfg)
    lightning = mapping(trainer_cfg.pop("trainer_kwargs", None), "m3gnet.trainer.trainer_kwargs")
    loader = mapping(trainer_cfg.pop("loader_kwargs", None), "m3gnet.trainer.loader_kwargs")
    lightning.update(
    ...
    trainer = MGLPotentialTrainer(
        ...
        data_mean=mean,
        data_std=std,
        trainer_kwargs=lightning,
        loader_kwargs=loader,
        **trainer_cfg,
    )
    native = work / "model"
    trainer.fit(dataset=dataset, save_path=native)
    output = Path(args.output).absolute()
    output.parent.mkdir(parents=True, exist_ok=True)
    base = work / "packed-model"
    created = Path(shutil.make_archive(str(base), "gztar", root_dir=work, base_dir="model"))
    shutil.move(str(created), str(output))
    return (
        "application/gzip",
        {"model_size_bytes": float(output.stat().st_size)},
        {"element_types": list(model.element_types), "work_dir": str(work)},
    )
