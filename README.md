...
```bash
git clone https://github.com/yezixin2023/mlipipe.git
cd mlipipe
...
```bash
mlipipe init my-project
cd my-project
mlipipe doctor
```

`mlipipe init` creates a minimal `project.yaml` and initializes local state. Add workflow nodes, inputs, parameters, dependencies, and routing policy before launching scientific work.
...
```bash
python -c "from shutil import copytree; copytree('examples/high_entropy_sulfide', 'mlipipe-replay-demo')"

mlipipe --project mlipipe-replay-demo init
mlipipe --project mlipipe-replay-demo list
mlipipe --project mlipipe-replay-demo run structure-replay --dry-run
mlipipe --project mlipipe-replay-demo run structure-replay
mlipipe --project mlipipe-replay-demo advance
mlipipe --project mlipipe-replay-demo status
```
...
MLIPipe ships a fixed set of scientific capability adapters. Each adapter validates inputs, plans execution, checks scientific completion, and collects results. MLIPipe core owns replay, DAG progression, and fresh-attempt retry semantics. Agent Skills provide supervision guidance; they do not replace numerical software or adapter code.

| Research capability | Capability ID | Agent Skill | Main scope | Execution |
|---|---|---|---|---|
| End-to-end workflow design | MLIPipe core | [`$mlip-workflow`](.agents/skills/mlip-workflow/SKILL.md) | DAG construction, evidence handoff, approval boundaries, and supervision | Supervision |
| High-entropy and SQS structures | `high-entropy-structure` | [`$high-entropy-structure`](mlipipe/plugins/high_entropy_structure/skill/SKILL.md) | Seeded `icet` SQS generation with explicit composition and supercell contracts | Local |
| PES sampling | `pes-sampling` | [`$pes-sampling`](mlipipe/plugins/pes_sampling/skill/SKILL.md) | DIRECT-based structure selection, LASP/SSW sampling, replay of existing sampling runs, and structure-set merging | Local; SSH-Slurm for scheduled LASP |
| DFT labeling and datasets | `dft-labeling` | [`$dft-labeling`](mlipipe/plugins/dft_labeling/skill/SKILL.md) | VASP preparation and static, relaxation, or AIMD labeling, followed by standardized labeled-dataset assembly | Local preparation/assembly; local or SSH-Slurm labeling/assembly |
| MLIP training and fine-tuning | `mlip-training` | [`$mlip-training`](mlipipe/plugins/mlip_training/skill/SKILL.md) | DeepMD, M3GNet/MatGL, CHGNet, and MACE training or fine-tuning with explicit references | Local; SSH-Slurm |
| ASE molecular dynamics | `ase-md` | [`$ase-md`](mlipipe/plugins/ase_md/skill/SKILL.md) | NVT Langevin or isotropic MTK NPT, supercells, checkpoints, and fresh-attempt restart | SSH-Slurm |
| LAMMPS molecular dynamics | `lammps-md` | [`$lammps-md`](mlipipe/plugins/lammps_md/skill/SKILL.md) | Deterministic input preparation, DeepMD/MACE/MatGL execution, collection, and binary restart | Local preparation; SSH-Slurm execution |
| MLIP benchmarking | `mlip-benchmark` | [`$mlip-benchmark`](mlipipe/plugins/mlip_benchmark/skill/SKILL.md) | Fresh or normalized static-PES evidence plus task-separated AIMD-reference RDF and transport metrics | Local; SSH-Slurm for fresh inference |
| Offline active learning | `active-learning` | [`$mlip-active-learning`](mlipipe/plugins/active_learning/skill/SKILL.md) | Uncertainty/risk-based screening, DFT-label selection, and round-wise declared-domain coverage and independent-accuracy assessment | Local selection and assessment; SSH-Slurm for large-scale inference |
| Ionic transport and dynamics | `ionic-transport` | [`$ionic-transport`](mlipipe/plugins/ionic_transport/skill/SKILL.md) | Trajectory/MSD analysis, diffusion, conductivity, Haven ratio, Arrhenius fitting, and bounded partial-RDF comparison | Local |
| Candidate ranking | `candidate-ranking` | [`$candidate-ranking`](mlipipe/plugins/candidate_ranking/skill/SKILL.md) | Ranking and top-k selection of candidates according to user-defined quantitative metrics | Local |
...
The voltage capability is orchestrated directly or through `$mlip-workflow`; it does not currently have a dedicated Agent Skill. Each specialist Skill lives in its capability's `skill/` folder under [`mlipipe/plugins/`](mlipipe/plugins/), beside the implementation. [`.agents/skills/`](.agents/skills/) provides relative directory links for Agent discovery and holds the standalone `mlip-workflow` Skill. `mlipipe inspect NODE` reports the node's resolved operation, effective approval requirement, and the operations supported by its built-in capability.

## Environments and HPC

Projects declare an optional backend profile and abstract resources such as CPUs, GPUs, memory, and wall time. If the profile is omitted, MLIPipe uses the configured default or sole cluster profile; set `backend_profile: auto` to request cross-cluster selection. SSH aliases, partitions, accounts, modules, executables, launchers, canonical data/model roots, and remote work roots belong in the user-local site configuration, normally `~/.mlipipe/site.yaml`, and site-owned templates. This keeps private infrastructure out of research repositories.

Review [`docs/CLUSTER_ENVIRONMENTS.md`](docs/CLUSTER_ENVIRONMENTS.md) and [`examples/site_templates/`](examples/site_templates/) before using a new cluster. Start with `mlipipe doctor`, inspect the dry-run plan, and validate a replay or bounded smoke case before scaling the same contract.

### Example agent requests

After initializing an MLIPipe project, you can describe a scientific task directly to a compatible AI agent. The agent can select the appropriate bundled Skill, or you can explicitly invoke one with `$skill-name`.
...
> Compare the completed AIMD and MACE trajectories at the same temperature using Li diffusion, conductivity, and the requested partial RDF. Reuse the trajectory metadata already collected by MLIPipe instead of asking me to copy or rename files.
...
| Command | Purpose |
|---|---|
| `mlipipe doctor` | Check project, built-in capability, state, and site configuration without mutation. |
| `mlipipe list`, `status`, `json` | Inspect workflow state in human- or machine-readable form. |
| `mlipipe inspect NODE`, `logs NODE` | Review a node, its capability, current state, diagnostics, and collected logs. |
| `mlipipe run NODE --dry-run` | Build the execution plan without launching work. |
| `mlipipe run NODE --approve` | Execute approval-gated work after reviewing the plan. |
| `mlipipe advance` | Observe submitted work, fetch declared outputs, run checks, and reconcile state; it never launches a new node. |
| `mlipipe retry NODE`, `stop NODE` | Create a fresh attempt or explicitly stop supported running work. |
| `mlipipe route ...` | Rank compatible models from versioned benchmark evidence and project policy. |
...
```bash
mlipipe --project . run train-model --dry-run
mlipipe --project . run train-model --approve
mlipipe --project . advance
mlipipe --project . status train-model
```
...
All Python implementation lives in [`mlipipe/`](mlipipe/). Start with the CLI,
then follow the query or execution services into a built-in capability.

| Location | Responsibility |
|---|---|
| `mlipipe/cli.py` | Command parsing and dispatch |
| `mlipipe/services/` | Read-only queries, approved execution and scheduler lifecycle |
| `mlipipe/plugins/__init__.py` | Explicit built-in capability registry |
| `mlipipe/plugins/<capability>/adapter.py` | Capability operation and lifecycle entrypoint |
| `mlipipe/plugins/<capability>/` | Operation contracts, scientific checks and runners |
| `mlipipe/plugins/model_runtime.py` | Shared lazy model-family and inference support |
| `tests/`, `examples/` | Regression tests and runnable examples |
| `mlipipe/plugins/<capability>/skill/` | Specialist Agent guidance and references beside its implementation |
...
MLIPipe is licensed under the [Apache License 2.0](LICENSE). For published research, cite the software version or commit using [`CITATION.cff`](CITATION.cff), together with the scientific methods, datasets, models, and external codes used by the executed capabilities.
