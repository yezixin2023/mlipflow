...
For `backend: ssh-slurm`, use only the implemented `bundled-vasp-static-v1` contract. Require one static structure, an SSH config alias, an existing `remote_root`, exact partition/QoS/nodes/tasks/time, a strict module list, `remote_python`, and exact `srun --ntasks=N vasp_std|vasp_gam|vasp_ncl`. Confirm `N` matches the approved resource count. The first approval creates a fresh remote directory, stages only the declared basenames, verifies each SHA-256, and submits one job. Never fetch POTCAR.
