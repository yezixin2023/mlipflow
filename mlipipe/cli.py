"""MLIPipe command-line interface."""
...
from .errors import MLIPipeError
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="local site.yaml (defaults to ~/.mlipipe/site.yaml only when HPC resolution needs it)",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    except (MLIPipeError, OSError, ValueError) as exc:
        code = exc.code if isinstance(exc, MLIPipeError) else "IO_OR_VALUE_ERROR"
        envelope = {
            "schema_version": 1,
            "ok": False,
            "command": args.command,
...
def dispatch(args: argparse.Namespace) -> dict[str, Any]:
    ...
    raise MLIPipeError(f"unsupported command: {command}")
