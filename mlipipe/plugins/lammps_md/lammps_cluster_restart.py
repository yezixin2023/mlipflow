...
if __package__:
    from . import lammps_cluster as base
else:
    import lammps_cluster as base
if __package__:
    from . import lammps_restart as restart_deck
else:
...
RESTART_STEP_MARKER = re.compile(r"MLIPIPE_RESTART_STEP=([0-9]+)")
...
def _restart_step(executable: Path, options: list[str], candidate: Path) -> int | None:
    ...
    probe = (
        ...
        "variable mlipipe_restart_step equal step\n"
        'print "MLIPIPE_RESTART_STEP=${mlipipe_restart_step}"\n'
    ).encode("utf-8")
    completed = subprocess.run(
