    ...
    if "analysis_script" in inputs:
        diagnostics.append(
            contracts._diagnostic(
                ...
                "analysis_script is packaged by MLIPipe and cannot be overridden",
            )
        )
    ...
    if not isinstance(python_executable, str) or not python_executable.strip():
        ...
        diagnostics.append(
            contracts._diagnostic(
                "ERROR",
                "resource.python_executable",
                "formal transport must use the same local Python interpreter that runs MLIPipe",
            )
        )
    else:
...
def _verify_smoke_manifest(
    ...
    expected_source_paths = {
        ...
        "mlipipe-transport-analysis-source": contracts._bundled_analysis_script(),
        "mlipipe-handoff-wrapper": contracts._MODULE_PATH.with_name("md_smoke_handoff.py").resolve(),
    }
