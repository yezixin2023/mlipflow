...
def _canonical_input(path: Path) -> dict[str, Any]:
    ...
    if value.get("contract") != "mlipipe/canonical-dataset-merge":
        return value
...
def _mace(root: Path, canonical: Mapping[str, Any], split: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for split_name, file_stem in PARTITIONS:
        ...
        for record in records_for_split(canonical, split, split_name):
            ...
            atoms.info.update({
                "mlipipe_record_id": record["record_id"],
                "mlipipe_source_structure_id": record["source_structure_id"],
                "mlipipe_calculation_id": record["calculation_id"],
                "mlipipe_ionic_step": int(record["ionic_step"]),
            })
            frames.append(atoms)
