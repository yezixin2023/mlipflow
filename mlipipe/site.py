...
in ``~/.mlipipe/site.yaml`` (or an explicitly supplied test/config path).
"""
...
DEFAULT_SITE_PATH = Path("~/.mlipipe/site.yaml")
...
def load_site_config(path: Path | None = None) -> SiteConfig:
    ...
    if not candidate.is_file():
        raise ConfigError(
            f"site config is missing: {candidate}; create ~/.mlipipe/site.yaml "
